...
func (s authHandlerSuite) TestRefreshEmailVerificationCode() {
	tests := []struct {
		name              string
		req               dto.CustomerRefreshEmailVerificationCode
		contentType       string
		expectedCode      int
		expectedResponses dto.BaseResponse
		service           funcRefreshEmailVerificationCode
	}{
		{
			name: "Valid",
			req: dto.CustomerRefreshEmailVerificationCode{
		...
		{
			name:         "Error because some field is not filled",
			req:          dto.CustomerRefreshEmailVerificationCode{},
			contentType:  echo.MIMEApplicationJSON,
			expectedCode: http.StatusBadRequest,
			expectedResponses: dto.BaseResponse{
	...
	for _, test := range tests {
		...
		s.handler.service = mockAuthService{
			funcRefreshEmailVerificationCode: test.service,
		}

		s.Suite.T().Run(test.name, func(t *testing.T) {
			if assert.NoError(t, s.handler.RefreshEmailVerificationCode()(ctx)) {
