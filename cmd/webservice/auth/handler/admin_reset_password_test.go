...
func (s authHandlerSuite) TestAdminResetPassword() {
	tests := []struct {
		name              string
		req               dto.AdminResetPassword
		contentType       string
		expectedCode      int
		expectedResponses dto.BaseResponse
		service           funcAdminResetPassword
	}{
		{
			name: "Valid",
			req: dto.AdminResetPassword{
			...
			service: func(ctx context.Context, req *dto.AdminResetPassword) error {
				return nil
			},
		},
		{
		...
		{
			name: "Error because service return error not found",
			req: dto.AdminResetPassword{
				Email:    "valid@gmail.com",
				Password: "valid123",
				Code:     "valid code",
	...
	for _, test := range tests {
		...
		s.handler.service = mockAuthService{
			funcAdminResetPassword: test.service,
		}

		s.Suite.T().Run(test.name, func(t *testing.T) {
			if assert.NoError(t, s.handler.AdminResetPassword()(ctx)) {
