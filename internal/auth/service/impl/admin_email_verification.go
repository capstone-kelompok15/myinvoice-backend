...
func (s *authService) AdminEmailVerification(ctx context.Context, req *dto.AdminEmailVerification) error {
	redisKey := fmt.Sprintf(dto.MerchantEmailVerifRedisKey, req.Email)
	...
	if err != nil {
		s.log.Warningln("[AdminEmailVerification] Error while getting the code on redis", err.Error())
		return err
	}
	...
	err = s.repo.AdminEmailVerification(ctx, req)
	if err != nil {
